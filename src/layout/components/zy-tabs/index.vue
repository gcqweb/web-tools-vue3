<template>
  <section class="zy-tabs" id="zy-tab">
    <a-tabs size="small" hide-add v-model:activeKey="currentTab" @tabClick="handleTabClick" @edit="onEdit"
      ...
      <a-tab-pane v-for="pane in list" :key="pane.path" :closable="pane.path!=='/index'">
        <template #tab>
          {{ pane.meta.title }}<a-badge v-if=" pane.meta.title==='聊天室'" :count="msgCount"></a-badge>
          <!--<a-badge count="25" /> -->
        </template>
      </a-tab-pane>
    </a-tabs>
    ...
    <a-dropdown class="close-tab" placement="bottomRight">
