...
const handleRequestError = (error) => {
    ...
    if (error.response) {
        ...
        switch (status) {
            case 400:
                console.error('参数校验出错:', error.response.data.message);
                ZyNotification.error(error.response.data.message || '参数校验出错')
                return Promise.reject(error.response.data.message ?? '参数json无法解析');
            case 401:
            ...
            case 500:
                console.error('服务器拒绝请求:', error.response.data.message);
                ZyNotification.error(error.response.data.message || '服务器拒绝请求')
                return Promise.reject({error: '服务器拒绝请求', message: error.response.data.message});
            default:
